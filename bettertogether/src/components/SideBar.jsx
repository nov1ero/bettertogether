...
const Icon = ({ styles, name, imgUrl, isActive, disabled, handleClick, isDarkMode }) => {
  ...
  const imgStyles = isLogo ? 'w-[120%] h-[120%]' : `w-1/2 h-1/2 ${isActive !== name ? 'grayscale' : ''}`;
...
const Sidebar = () => {
  ...
  return (
    <div className="flex justify-between items-center flex-col sticky top-5 h-[93vh]">
      <div className={`flex justify-between items-center flex-col rounded-[7px] sticky  h-[9.8vh] ${isDarkMode ? 'bg-[#1c1c24]' : 'bg-[#e6e6e6]'}`}>
        <Link to="/home">
          <Icon styles="w-[80px] h-[70px] bg-[#2c2f32]" imgUrl={logo} />
        </Link>
      </div>
